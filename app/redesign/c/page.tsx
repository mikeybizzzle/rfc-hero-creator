...
export default function ProtoC() {
  return (
    <div className="proto-c min-h-screen pb-10">
          ...
          >
            {navLinks.map(({ label, href }) => (
              <Link
                key={href}
                href={href}
                className="shrink-0 rounded-lg px-3 py-2 text-[13px] font-extrabold text-muted transition-colors hover:bg-surface hover:text-cream"
              >
                {label}
              </Link>
            ))}
      ...
      <div className="mx-auto max-w-6xl px-4">
        <section className="pt-9 sm:pt-12">
          <div className="lz-red-bar relative mb-4 pl-4">
              ...
              {paths.map((o, i) => {
                ...
                return (
                  <Link
                    key={o.href}
                    href={o.href}
                    className="lz-cream-card lz-card-hover group flex h-full snap-start flex-col overflow-hidden"
                  >
        ...
        <section className="pt-10 sm:pt-14">
          <div className="lz-red-bar relative mb-3 pl-4">
            <h2 className="lz-display text-[clamp(25px,4vw,34px)]">The wall</h2>
            <p className="mt-1 text-[15px] text-muted">
      ...
      <footer className="mt-14 border-t border-line bg-raised/55">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-7 text-[13px] text-muted sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2.5">
            <span className="lz-display text-base text-gold">RfC alliance</span>
