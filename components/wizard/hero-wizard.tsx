...
import { heroCardSheetPrompt, type Rank } from "@/lib/prompts";
...
export function HeroWizard({
  ...
  async function handlePrompt() {
    const promptText = heroCardSheetPrompt(rankSel, heroName, details, true);
    if (promptStage === 0) {
      try {
        ...
        >
          ...
          <div className="mt-2.5 flex items-center gap-2.5">
            <button
              ...
              className="min-h-11 flex-1 rounded-xl border border-line bg-raised px-3 py-2.5 text-xs font-extrabold tracking-[.5px] text-gold transition-colors hover:border-gold"
            >
              {t("uploadPhoto")}
            </button>
            {photo && (
              <span className="relative h-11 w-11 shrink-0">
                ...
                <img
                  src={photo}
                  alt={t("uploadPhoto")}
                  className="h-11 w-11 rounded-lg border border-gold object-cover"
                />
                <button
                  type="button"
                  onClick={removePhoto}
                  aria-label={t("removePhoto")}
                  className="absolute -right-1.5 -top-1.5 grid h-5 w-5 place-items-center rounded-full border border-line bg-bg text-[10px] text-cream"
                >
                  &#10005;
                </button>
              </span>
